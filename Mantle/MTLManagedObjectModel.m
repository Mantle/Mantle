...
// This coupling is needed for backwards compatibility in MTLManagedObjectModel's deprecated
// methods.
...
// receiver, and continuing up until (but not including) MTLManagedObjectModel.
//
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
	...
	while (!stop && ![cls isEqual:MTLManagedObjectModel.class]) {
		unsigned count = 0;
...
- (void)mergeValueForKey:(NSString *)key fromModel:(MTLManagedObjectModel *)model {
...
- (void)mergeValuesForKeysFromModel:(MTLManagedObjectModel *)model {
